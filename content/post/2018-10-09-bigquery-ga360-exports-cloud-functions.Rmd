...
There are lots use cases for Cloud Functions, some of my favourites are to create [mini APIs](https://cloud.google.com/functions/docs/tutorials/http), [load BigQuery with files hitting Cloud Storage](https://cloud.google.com/community/tutorials/cloud-functions-avro-import-bq), or [creating Slack commands](https://cloud.google.com/functions/docs/tutorials/slack)

### Scheduler vs Event based triggers

One major benefit is that it helps move you from a *schedule based* mindset (such as running scripts on a schedule/cron) to an *event based* one.  For example, the GA360 exports to BigQuery usually happen before 0900 for my clients, but that is not guaranteed.  If you have all your data processing set for a schedule of 0910 say, its going to break if those exports are late.  
...
Also cloud functions only work on the same project they are deployed to, so you will need to put it in the same GCP project as the BigQuery exports. 
...
Create a partitioned table within the dataset called `ga_sessions`.  You don't have to specify a schema as that will be created when the first data hits it. You could choose to require a partition filter if you have a lot of data, to make sure people don't query a lot of data by mistake. 
...
In the example below, adapated from [here](https://googlecloudplatform.github.io/google-cloud-python/latest/bigquery/index.html
), it then creates the BigQuery job to copy.  

Note here we do not wait for the job to finish - we are in an event style mindset, so just want to trigger and finish.  Instead just return the jobId, that you can examine in the BigQuery logs if you need to.
...
You may need to then install the `gcloud beta` componenets to deploy the python.  See the [guide](https://cloud.google.com/functions/docs/quickstart) here.
...
You need to specify the function that will trigger.  In our case this is the function that recieves the data, `def bq_to_bq(data, context)`.  We also need to specify its a Python file, and what the trigger will be:
...
Hopefully this helps you integrate Cloud Functions into your own workflows.  You can see that the code is minimal and for the right type of job give minimal upkeep without too much development work.  Whilst not suited to every task, they have quickly become a great tool in my toolbox for moving data around.
