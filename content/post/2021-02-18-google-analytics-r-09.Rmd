---
title: 'googleAnalyticsR 0.9.0 - The Big Release. GA4 API, automatic shiny dashboarding, improved UI'
author: Mark Edmondson
date: 2021-02-21T09:29:50+01:00
url: googleanalyticsr-090-release
...
But for `googleAnalyticsR v0.9.0` there are so many cool new things that I think it deserves a bit more limelight.  They are mostly the result of my R programming improving over the almost five years since [version 0.1.1](https://github.com/MarkEdmondson1234/googleAnalyticsR/releases/tag/v0.1.1), the cool new tools available in the R ecosystem created by the R community and the new capabilities of the Google Analytics APIs we're connecting to.
...
Below is an example workflow using the GA4 APIs - the use case here is to analyse the first and last touch of users.  It incorporates some of the new features of the API, such as the new filters, creating your own dimensions and ordering the results.  Its easier to use, so encourages more 'flow' in your analysis of the data.
...
ga_data(
  ...
  date_range = last_year,
  dimensions = "firstUserSource", 
  dimensionFilter = ip_filter, # apply filter
  limit = -1) # all results
...
ga_data(
  ...
  dimensions = 
    c("unifiedScreenName",
      lastMediumSource = "sessionMedium/sessionSource"), #dynamic dimension
  dimensionFilter = ga_data_filter(twit & me & nottwit), # combine filters
...
This has wide applications to send in analytics hits with R.  I'm thinking of using it to send some anonymous statistics of usage to a googleAnalytics GA4 account, or to add customer segments to GA4 BigQuery exports using R to model the clusters. 
...
I hope googleAnalyticsR now provides an easier framework for people to share nice Shiny themes on top of the ga_models available, which can then be combined as user's would like - the main use case for these functions is quick iteration between styles and quicker access to advanced GA applications.
