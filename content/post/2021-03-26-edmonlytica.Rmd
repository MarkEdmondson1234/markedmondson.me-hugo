...
It sends it to the GTM server side endpoint `/edmondlytica` reading the event name from the GTM trigger the tag is set up for.
...
I now create an instance of my tag and choose what triggers it - for me pageviews and external link triggers but you could add any other GTM trigger you have created - the tag will capture the event details for that trigger. 

![Edmonlytica GTM Browser-side tag](/images/edmonlytica-browser-tag.png)
...
I want to see my data in a Shiny dashboard. This app is deployed to Cloud Run as well. It includes visualizations of hit trends, top referrers and landing pages and some analysis tools such as forecasts and anomaly detection. 
...
I hope it also demonstrates who the tools such as GA360 and GA4 can also be complimented with your own data streams for not too much development time, and look forward to the publication of pre-made GTM tags and clients to help even those with no coding experience benefit.  
